...
and writes them to `prediction_<size>.csv` (`frame_no,ball_x,ball_y`; `live.csv`
...
| Ball speed is 2 px/frame median, 16 at p99, with one 534px "teleport" that is a scene cut | Tune the Kalman filter to roughly 2 to 3 px/frame, and set an outlier gate at 20px to reject false hits on the HUD, radar, or players. |
...
2.83 and a mean of 4.43, with 92.4% of frames within 10px and 97.7% within 25px,
